...
// Ultimo sign-in do app via beta API
...
async function getServicePrincipalByAppId(appId, token) {
  try {
    ...
    const res = await graphGet(
      "/servicePrincipals?$filter=appId eq '" + safeId + "'&$select=displayName,appId,id,servicePrincipalType,accountEnabled,appRoleAssignedTo,publisherName,homepage,replyUrls",
      token
    );
...
// Workloads inferidos pelas permissoes
...
async function collectApps(token) {
  ...
  var chunkSize = 10; // menor chunk para incluir SP lookup
  var appsEnriched = [];

  for (var i = 0; i < apps.length; i += chunkSize) {
    var chunk = apps.slice(i, i + chunkSize);

    // Busca owners, sign-ins e service principals em paralelo por chunk
    ...
    for (var j = 0; j < chunk.length; j++) {
      ...
      chunk[j]._lastSignIn = signInsChunk[j]; // array de ate 3 sign-ins
...
switchMainTab('writefiles', this)
  ...
  // ── Tooltips ──────────────────────────────────────────────────────────────
  var RISK_TOOLTIPS = {
    "Mail.ReadWrite": "CRITICO — Le e modifica todos os e-mails da org. Atacante pode ler, apagar e alterar mensagens.",
    "Mail.Send": "CRITICO — Envia e-mails como qualquer usuario. Pode ser usado para phishing interno.",
    "Files.ReadWrite.All": "CRITICO — Le e escreve em todos os arquivos SharePoint/OneDrive de todos os usuarios.",
    "Directory.ReadWrite.All": "CRITICO — Le e modifica todo o diretorio Azure AD: usuarios, grupos, roles.",
    "User.ReadWrite.All": "CRITICO — Cria, edita e deleta qualquer usuario. Pode ser usado para escalar privilegios.",
    ...
    "Application.ReadWrite.All": "CRITICO — Cria e modifica qualquer aplicacao, incluindo secrets e permissoes.",
    "Group.ReadWrite.All": "CRITICO — Cria e modifica todos os grupos, incluindo grupos de seguranca e Teams.",
    "full_access_as_app": "CRITICO — Acesso total ao Exchange Online como aplicacao, sem restricoes.",
    ...
    "Directory.Read.All": "ALTO — Le todo o diretorio AD: usuarios, grupos, roles, dispositivos.",
    ...
    "User.Read": "NORMAL — Le apenas o perfil do usuario que autorizou. Escopo limitado.",
  ...
  function buildAppRow(a, tabPrefix) {
      ...
      : allPerms.map(function(p) {
          ...
          return '<div class="pb-wrap">' +
            '<span class="pb" style="border-color:' + riskColor[risk] + ';color:' + riskColor[risk] + (isWrite ? ';font-weight:700' : '') + '">' +
              '<span class="pt">' + (isApp ? "APP" : "DEL") + '</span> ' + label + (isWrite ? ' ✏️' : '') +
            '</span>' +
    ...
    if (signIns && signIns.length > 0) {
      ...
      var stalenessColor = daysAgo > 90 ? "#ef4444" : daysAgo > 30 ? "#f59e0b" : "#4ade80";
      var stalenessLabel = daysAgo === 0 ? "Hoje" : daysAgo + " dias atras";
      lastUsoHtml = '<div class="audit-wrap">' +
        '<div class="audit-item"><span class="audit-action" style="color:#c8d8e8;font-weight:600">Ultimo sign-in detectado</span><span style="color:' + stalenessColor + ';font-family:JetBrains Mono,monospace;font-size:11px">' + stalenessLabel + '</span></div>' +
        '<div class="audit-item"><span class="audit-action">Data e hora</span><span class="audit-time">' + fmtDate(last.createdDateTime) + '</span></div>' +
        '<div class="audit-item"><span class="audit-action">Usuario</span><span style="color:#38bdf8;font-size:11px">' + escapeHtml(last.userDisplayName || last.userPrincipalName || "N/A") + '</span></div>' +
        ...
        '<div class="audit-item"><span class="audit-action">Cliente usado</span><span style="color:#94a3b8;font-size:11px">' + escapeHtml(last.clientAppUsed || "N/A") + '</span></div>' +
        ...
        (last.location ? '<div class="audit-item"><span class="audit-action">Localizacao</span><span style="color:#94a3b8;font-size:11px">' + escapeHtml((last.location.city || "") + ", " + (last.location.countryOrRegion || "")) + '</span></div>' : '') +
        (signIns.length > 1 ? '<div class="audit-title" style="margin-top:10px">Usos anteriores:</div>' +
          signIns.slice(1).map(function(si) {
            return '<div class="audit-item"><span class="audit-action">' + fmtDate(si.createdDateTime) + '</span><span style="color:#3a5068;font-size:10px">' + escapeHtml(si.userDisplayName || si.userPrincipalName || "N/A") + ' — ' + escapeHtml(si.ipAddress || "") + '</span></div>';
          }).join("") : '') +
      '</div>';
    } else {
      lastUsoHtml = '<div class="audit-wrap">' +
        '<div class="audit-item"><span class="audit-action">Nenhum sign-in encontrado</span><span style="color:#ef4444;font-size:11px">App possivelmente inativo ou usa fluxo client_credentials</span></div>' +
        '<div class="audit-item" style="color:#3a5068;font-size:11px">Sign-ins do tipo client_credentials (automacao sem usuario) nao aparecem nos logs de sign-in padrao. Verifique os Azure Monitor Workbooks para fluxos de app-only.</div>' +
      '</div>';
    }

    // Onde/como o app e usado
    ...
    var writePermsHtml = writePerms.length > 0
      ? writePerms.map(function(p) {
          var risk = classifyPerm(p.name);
          return '<div class="audit-item"><span class="audit-action" style="color:' + riskColor[risk] + ';font-weight:600">✏️ ' + escapeHtml(permLabel(p)) + '</span><span style="color:#3a5068;font-size:10px">' + escapeHtml(p.resource || "") + '</span></div>';
        }).join("")
      : '<div class="audit-item"><span style="color:#4ade80">✅ Nenhuma permissao de escrita</span></div>';

    var usoHtml = '<div class="audit-wrap">' +
      '<div class="audit-title">Workloads identificados pelas permissoes:</div>' +
      '<div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:12px">' + workloadsHtml + '</div>' +
      '<div class="audit-title">Permissoes de escrita (Write):</div>' +
      writePermsHtml +
      (sp ? '<div class="audit-title" style="margin-top:10px">Service Principal:</div>' +
        ...
        '<div class="audit-item"><span class="audit-action">Homepage</span><span style="color:#60a5fa;font-size:10px">' + escapeHtml(sp.homepage || "N/A") + '</span></div>' +
    ...
    var riscoHtml = '<div class="audit-wrap">' +
      '<div class="audit-item"><span class="audit-action" style="color:#c8d8e8;font-weight:600">Classificacao de Risco</span><span style="color:' + riskLevelColor + ';font-family:JetBrains Mono,monospace;font-weight:700;font-size:14px">' + a._riskLevel + '</span></div>' +
      ...
      '<div class="audit-item"><span class="audit-action">Owner</span><span style="color:' + (owners.length === 0 ? "#ef4444" : "#4ade80") + '">' + (owners.length === 0 ? "Sem owner (+20 pts risco)" : owners.length + " owner(s)") + '</span></div>' +
      '<div class="audit-item"><span class="audit-action">Uso recente</span><span style="color:' + (signIns && signIns.length > 0 ? "#4ade80" : "#3a5068") + '">' + (signIns && signIns.length > 0 ? "Sim — " + fmtDate(signIns[0].createdDateTime) : "Nao detectado") + '</span></div>' +
      '<div class="audit-title" style="margin-top:10px">Permissoes criticas detectadas:</div>' +
      (writePerms.length > 0 ? writePerms.map(function(p) {
        var risk = classifyPerm(p.name);
    ...
    return '<tr class="ar' + (isRisky ? " risky" : "") + '" onclick="toggle(\'rx-' + uid + '\')">' +
      '<td><div class="app-name">' + escapeHtml(a.displayName || "—") +
        ...
        (writePerms.length > 0 ? '<span class="write-badge">✏️ Write</span>' : '') +
        ((a.secrets && a.secrets.length > 0) ? '<span class="secret-badge">' + a.secrets.length + ' secret(s)</span>' : '') +
        (a.notes ? '<span class="notes-badge">📝</span>' : '') +
  ...
  var changesHtml = changes24h.length === 0
    ...
    : changes24h.slice().reverse().map(function(c) {
        ...
        return '<div class="change-item" style="border-left-color:' + color + '"><div class="change-top"><span>' + icon + '</span><span class="change-msg">' + escapeHtml(c.message) + '</span><span class="change-time">' + fmtDate(c.detectedAt) + '</span></div>' + pd + '</div>';
      }).join("");
  ...
  var secretAlertsHtml = allSecretIssues.length === 0
    ...
    : allSecretIssues.map(function(item) {
        ...
        return '<div class="change-item" style="border-left-color:' + color + '"><div class="change-top"><span>' + (item.secret.status === "expirada" ? "🚨" : "⚠️") + '</span><span class="change-msg"><strong>' + escapeHtml(item.appName) + '</strong> — ' + escapeHtml(item.secret.displayName) + '</span><span class="change-time" style="color:' + color + '">' + label + '</span></div><div style="font-size:11px;color:#3a5068;margin-top:4px;padding-left:20px">Criada: ' + fmtDateOnly(item.secret.startDate) + ' → Expira: ' + fmtDateOnly(item.secret.endDate) + '</div></div>';
      }).join("");
...
'.sl{font-size:9px;color:#3a5068;margin-top:3px;text-transform:uppercase;letter-spacing:.5px}' +
...
'.app-name{display:flex;align-items:center;gap:5px;flex-wrap:wrap;color:#c8d8e8;font-weight:500}' +
...
'.write-badge{font-size:9px;font-weight:700;padding:1px 6px;border-radius:10px;background:#ef444420;color:#ef4444;border:1px solid #ef444440;font-family:"JetBrains Mono",monospace;white-space:nowrap}' +
...
'.notes-badge{font-size:9px;padding:1px 5px;border-radius:10px;background:#4ade8020;color:#4ade80;border:1px solid #4ade8040;font-family:"JetBrains Mono",monospace}' +
'.rl-badge{font-size:9px;font-weight:700;padding:1px 6px;border-radius:10px;border:1px solid;font-family:"JetBrains Mono",monospace;white-space:nowrap}' +
...
'.sugg-wrap{display:flex;flex-direction:column;gap:10px}.sugg-ok{font-size:12px;color:#4ade80;padding:8px;background:#061a10;border-radius:6px;font-family:"JetBrains Mono",monospace}' +
'.sugg-section{display:flex;flex-direction:column;gap:4px}.sugg-title{font-size:10px;font-weight:700;margin-bottom:6px;font-family:"JetBrains Mono",monospace;text-transform:uppercase;letter-spacing:1px}' +
...
'.sugg-remove code{color:#ef4444;border:1px solid #ef444430}.sugg-add code{color:#4ade80;border:1px solid #4ade8030}.sugg-item span{color:#94a3b8;line-height:1.5}' +
...
'.expand-tabs{display:flex;gap:4px;margin-bottom:12px;flex-wrap:wrap}' +
'.etab{font-size:10px;padding:4px 10px;border-radius:5px;cursor:pointer;background:transparent;color:#3a5068;border:1px solid #1a2840;font-family:"JetBrains Mono",monospace;transition:all .2s;white-space:nowrap}' +
...
'.pb{display:inline-flex;align-items:center;gap:3px;font-size:10px;padding:2px 7px;border-radius:5px;border:1px solid;font-family:"JetBrains Mono",monospace}' +
...
'.secret-name{font-size:11px;color:#c8d8e8;font-family:"JetBrains Mono",monospace;flex:1}.secret-dates{font-size:10px;color:#3a5068}.secret-status{font-size:10px;font-weight:700;font-family:"JetBrains Mono",monospace}' +
...
'.audit-item{display:flex;align-items:flex-start;gap:10px;padding:6px 8px;background:#0a1525;border-radius:5px;flex-wrap:wrap}' +
'.audit-action{font-size:11px;color:#c8d8e8;flex:1;min-width:120px}.audit-actor{font-size:10px;color:#38bdf8;font-family:"JetBrains Mono",monospace}.audit-time{font-size:10px;color:#3a5068}' +
'.wb{display:inline-flex;align-items:center;font-size:11px;padding:3px 10px;border-radius:6px;background:#0a1f30;border:1px solid #1e4060;color:#38bdf8;font-family:"JetBrains Mono",monospace}' +
...
'.change-top{display:flex;align-items:center;gap:6px;flex-wrap:wrap}.change-msg{font-size:11px;color:#c8d8e8;flex:1}.change-time{font-size:10px;color:#3a5068;white-space:nowrap;font-family:"JetBrains Mono",monospace}' +
...
'.no-changes{font-size:11px;color:#2a4060;text-align:center;padding:20px 0;font-family:"JetBrains Mono",monospace}.empty-tab{font-size:12px;color:#2a4060;text-align:center;padding:40px 0;font-family:"JetBrains Mono",monospace}' +
...
'<div class="hdr"><div class="hdr-brand"><div><h1>// APP MONITOR</h1><div class="tid">' + session.tenantId + '</div></div></div>' +
'<div class="hdr-right"><div class="live"><div class="dot"></div>LIVE</div><div class="cd" id="cd">NEXT: <strong id="timer">5:00</strong></div>' +
'<div class="ri" id="ri" title="Atualizar agora" onclick="forceRefresh()">↻</div><div class="upd">UPD: <span id="upd">agora</span></div></div></div>' +
...
'<div class="card-header"><div class="card-title" id="tabLabel">Todas as Aplicacoes (' + allApps.length + ')</div>' +
...
'<div><div class="side-card"><div class="side-title"><span>⚠️ Mudancas (24h)</span><span style="color:#2a4060">' + changes24h.length + '</span></div><div id="changesPanel">' + changesHtml + '</div></div>' +
...
'        return\'<div class="change-item" style="border-left-color:\'+color+\'"><div class="change-top"><span>\'+icon+\'</span><span class="change-msg">\'+esc(c.message)+\'</span><span class="change-time">\'+new Date(c.detectedAt).toLocaleString("pt-BR")+\'</span></div>\'+pd+"</div>";' +
'      }).join("");}' +
...
'function showToast(c){var area=document.getElementById("toast-area");var icons={critico:"🚨",aviso:"⚠️",melhora:"✅",info:"ℹ️"};var t=document.createElement("div");t.className="toast "+(c.severity||"info");t.innerHTML=\'<span>\'+(icons[c.severity]||"ℹ️")+\'</span><span>\'+esc(c.message)+\'</span><span class="tc" onclick="this.parentElement.remove()">×</span>\';area.appendChild(t);setTimeout(function(){if(t.parentElement)t.remove();},10000);}' +
